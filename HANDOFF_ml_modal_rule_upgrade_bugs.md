...
2. **Manual-test data-setup scripts** — ✅ **DONE (built; live-run pending).** New quickstart module `scripts/quickstart/modules/ml_coverage_loss/` (`index.ts` orchestrators `seedMlCoverageLossState` / `teardownMlCoverageLossState` / `verifySeededUpgrades`, `constants.ts` fixtures, `api.ts` raw calls, `rule_assets.ts` doc builder) + README example. Seeds a legacy ML job (`v2_windows_rare_metadata_user`, `groups:['security']`) and **4 upgradeable prebuilt-rule fixtures**: A coverage-loss (→NON_SOLVABLE), B customized coverage-loss (→NON_SOLVABLE, `CustomizedValueCanUpdate`), C non-affected ML `_ea`→`_ea` (→NONE), D non-ML query rule (→ML field ABSENT; proves non-ML rules aren't blocked). Mechanics: bulk-write v1 `security-rule` assets **before** perform-install (so the real Fleet package isn't fetched), install SPECIFIC_RULES, patch B, publish v2 assets; `verifySeededUpgrades` logs a PASS/FAIL table via `reviewRuleUpgrade`. No test-package imports (glue reimplemented on the quickstart `kbnClient`+`esClient`, `legacy_actions` idiom). Idempotent (self-teardown first). `createMlJob:false` flag for basic-license stacks (job creation needs trial+; the conflict repro is content-based and works without it). Gates: eslint + IDE TS diagnostics clean on all files. **Temporarily wired** into `scratchpad.ts` (marked `TEMPORARY (revert before committing)` — the demo imports + lists/exceptions clients were trimmed so it still compiles). **Still pending:** actually running it against a live stack (see §3). Note: `bulkWriteRuleAssets` throws on `response.errors`, so a mapping/schema issue on the direct SO write will surface loudly there — the most likely spot to need a tweak once run live.
3. **Test on a live stack** — manual test on **both** Enterprise and a basic license (via the §2 scripts); run the E2E on CI.
4. **Clean up branch history before PR** — drop/squash the superseded `b441bc3` + `7139f98`; drop the unrelated `87179581` kbn-babel-preset commit; commit the handoff doc (currently staged); consider squashing the `Fix 239884: *` commits into one. **Revert the temporary `scratchpad.ts` wiring** (the two `TEMPORARY` blocks) — but **do** commit the `modules/ml_coverage_loss/` module + README additions.
